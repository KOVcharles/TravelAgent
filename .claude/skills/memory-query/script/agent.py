...
class MemoryQueryAgent(AgentBase):
    ...
    async def reply(self, x: Optional[Union[Msg, List[Msg]]] = None) -> Msg:
        ...
        if self.memory_manager:
            ...
            # 获取短期对话，优先使用协调器上下文，缺失时从 MemoryManager 读取
            if not recent_dialogue:
                recent_dialogue = self.memory_manager.short_term.get_recent_context(n_turns=5)
        ...
        # 格式化近期对话
        recent_dialogue_text = self._format_recent_dialogue(recent_dialogue)
...
"""
        ...
        try:
            ...
            result = {
                ...
                "memory_sources": {
                    ...
                    "has_recent_dialogue": bool(recent_dialogue),
    ...
    def _format_recent_dialogue(self, recent_dialogue: List[Dict]) -> str:
        """格式化近期短期对话"""
        if not recent_dialogue:
            return "（暂无近期对话）"
        ...
        return "\n".join(lines) if lines else "（暂无近期对话）"
    ...
    def _format_preferences(self, preferences: Dict) -> str:
        ...
        for key, value in preferences.items():
            if not value:
                continue
            display_name = pref_names.get(key, key)
