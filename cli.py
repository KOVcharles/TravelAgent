...
class HommeyCLI:
    ...
    async def initialize_system(self):
        ...
        with self.console.status("Initializing...", spinner="dots"):
            ...
            self.mcp_manager = MCPManager(
                ...
                auto_connect=mcp_config.auto_connect,
                connect_timeout=mcp_config.connect_timeout,
                circuit_breaker=self.circuit_breaker,
            )
            # 注入 mcp_manager 到 lazy_registry（供 mcp-tool Skill 使用）
            lazy_registry.mcp_manager = self.mcp_manager

            # 异步初始化 MCP 连接
            if mcp_config.get_enabled_servers():
                mcp_results = await self.mcp_manager.initialize()
    ...
    def _generate_human_response(self, results: list) -> bool:
        ...
        for result in results:
            ...
            # MCP 工具调用
                ...
                if tool_result:
                    self.console.print(f"\n[bold]📁 MCP 工具结果 ([cyan]{server}.{tool}[/cyan])[/bold]")
                    # 截断过长结果
                    if len(str(tool_result)) > 500:
                        self.console.print(str(tool_result)[:500] + "...")
                    else:
                        self.console.print(str(tool_result))
                    current_agent_shown = True
                elif tool_error:
                    self.console.print(f"\n[bold red]❌ MCP 工具错误 ([cyan]{server}.{tool}[/cyan])[/bold red]")
                    self.console.print(f"  {tool_error}")
                    current_agent_shown = True
    ...
    def show_status(self):
        ...
        memory_table.add_row(
            "短期记忆",
            f"{short_term_stats['total_messages']} 条消息 ({short_term_stats.get('backend', 'unknown')})"
        )
        long_term_backend = MEMORY_CONFIG.get("long_term", {}).get("backend", "unknown")
        long_term_path = getattr(self.memory_manager.long_term, "file_path", None)
        long_term_status = f"{long_term_stats['total_trips']} 次行程 ({long_term_backend})"
        if long_term_path:
            long_term_status += f"\n{long_term_path}"
        memory_table.add_row(
            "长期记忆",
