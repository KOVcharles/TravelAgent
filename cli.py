...
class AligoCLI:
    ...
    def _display_results(self, result_data: dict):
        ...
        if not results:
            # 没有任何智能体被调用（理论上不会走到这里，process_query 已兜底）
            self.console.print("✓ 好的，我已收到。", style="green")
            self.console.print("\n💡 你可以试试：", style="dim")
            ...
            self.console.print("  • 随便聊聊：「今天天气真好」", style="dim")
        else:
            # 情况2: 有智能体被调用，生成人性化回复
    ...
    async def _handle_chitchat(self, user_input: str):
        """闲聊兜底：当无业务意图时，调用 ChitchatAgent 生成友善回复"""
        from agentscope.message import Msg
        import importlib.util

        agent = None
        try:
            # 尝试从懒加载注册器获取 chitchat agent
        ...
        if agent is None:
            # 注册器中没有，动态加载
            try:
                script_path = os.path.join(project_root, ".claude", "skills", "chitchat", "script", "agent.py")
        ...
        if agent is None:
            # 终极兜底：完全本地回复
            self.console.print()
            self.console.print("💬 嗯嗯，我听着呢～有什么出行相关的问题需要帮忙吗？😊", style="bold white")
            self.console.print()
            self.memory_manager.add_message("assistant", "嗯嗯，我听着呢～有什么出行相关的问题需要帮忙吗？😊")
            return
        ...
        try:
            ...
            logger.warning(f"Chitchat fallback failed: {e}")
            data = {"response": "嗯嗯，我听着呢～有什么出行相关的问题需要帮忙吗？😊", "source": "fallback"}

        reply_text = data.get("response", "") if isinstance(data, dict) else str(data)
        self.console.print()
        self.console.print(f"💬 {reply_text}", style="bold white")
        self.console.print()
        self.memory_manager.add_message("assistant", reply_text)
